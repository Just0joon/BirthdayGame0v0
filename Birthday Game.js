...
$(document).ready(() => {
	...
	function start() {
		...
		if (modifiers.fire.isOn) {
			if (player.Character != 'Kikimama') player.addEffect('burning', Infinity, modifiers.fire.effectStrength, 5)
			if (enemy.Character != 'Kikimama') enemy.addEffect('burning', Infinity, modifiers.fire.effectStrength, 5)
		}
	...
	class Character {
		...
		addEffect(type, duration, strength, max, shouldSum = false) {
			const existingEffect = this.effects.find(eff => eff.type === type);
	...
	class Kikimama extends Character {
		...
		specialAttack1() {
			if (this.mana >= this.attack1Mana) {
				if (this.enemy.Character != "Kikimama" && !modifiers.fire.isOn) {
				...
				if (modifiers.doublePunch.isOn) {
					setTimeout(() => {
						if (this.enemy.Character != "Kikimama" && !modifiers.fire.isOn) {
							this.enemy.addEffect('burning', 3, 1, 3)
						}
						if (this.enemy.mana < 1) {
							this.attack(this.boost ? 6 : 5)
						} else {
		...
		specialAbility() {
			if (this.maxMana > 3) {
				...
				if (this.enemy.Character != "Kikimama") {
					this.breakEnemyArmor(5)
					this.attack(15)
					this.enemy.addEffect('burning', 1, 1, 1, true)
				}
				this.heal(1)
				this.updateUI()
				this.updateEnemyUI()
	...
	$('.modifier').click(function() {
		...
		$(this).css('transform', 'translateX(-2vw)')
